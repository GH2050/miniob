...
#include "sql/operator/join_logical_operator.h"//my4
...
RC PredicatePushdownRewriter::rewrite(std::unique_ptr<LogicalOperator> &oper, bool &change_made)
{
  ...
  std::unique_ptr<LogicalOperator> &child_oper = oper->children().front();//my4
  // if (child_oper->type() != LogicalOperatorType::TABLE_GET || child_oper->type() != LogicalOperatorType::JOIN) {
  if (child_oper->type() != LogicalOperatorType::TABLE_GET && child_oper->type() != LogicalOperatorType::JOIN) {
    return rc;//my4
  }

// auto table_get_oper = static_cast<TableGetLogicalOperator *>(child_oper.get());//my4 
  ...
  // 可以被下推的表达式保存在pushdown_exprs里，且已经从原来的逻辑计划中删除
  ...
  if (child_oper->type() == LogicalOperatorType::TABLE_GET) {
    ...
    if (left_join_child_type == LogicalOperatorType::JOIN && right_join_child_type == LogicalOperatorType::TABLE_GET) {
      ...
      // pushdown里存的是需要下推到左join前的表达式,
      // right_child_exprs存的是需要保留的与右表有关系的表达式
      // 处理右表
      ...
      std::unique_ptr<ConjunctionExpr> conjunction_expr(
          new ConjunctionExpr(ConjunctionExpr::Type::AND, pushdown_exprs));
      std::unique_ptr<PredicateLogicalOperator> left_new_oper =
          std::unique_ptr<PredicateLogicalOperator>(new PredicateLogicalOperator(std::move(conjunction_expr)));
      ...
      // 左右子算子都是talbe get的话, 不需要pushdown
    ...
    std::unique_ptr<Expression> &expr, std::vector<std::unique_ptr<Expression>> &pushdown_exprs)
{
  ...
  if (expr->type() == ExprType::CONJUNCTION) {
    ...
    if (comp >= NO_OP) {// != EQUAL_TO) {//my4 增加可比较操作
